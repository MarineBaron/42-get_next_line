...
/*   Updated: 2018/01/27 14:47:11 by mbaron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(int argc, char *argv[])
{
	...
	while (i < argc - 1)
	{
		...
		ft_strcpy(output_file[i], "output_");
		ft_strcat(output_file[i], argv[i + 1]);
		if (!(fd_output[i] = open(output_file[i], O_WRONLY)))
			return (put_error("File to write can't be opened !"));
		gnl[i] = -1;
		if (!(str[i] = ft_strnew(4096)))
